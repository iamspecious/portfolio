...
|---|---|---|
| [Error Log Interpreter](https://iamspecious.github.io/portfolio/error-interpreter.html) | Paste a browser console error, Vite build failure, or stack trace — get a plain-English breakdown that distinguishes where the error *surfaces* from where it *originated* | Live |
| [Debug Helper](https://iamspecious.github.io/portfolio/debug-helper.html) | Structured debugging coach with three modes: I have an error, code runs but does the wrong thing, and total silence — built especially for the silent-failure case | Live |
| [LoreBySpec Creator Tool](https://iamspecious.github.io/portfolio/creator-tool.html) | Two-mode content assistant for my YouTube channel: video idea generation and comment reply drafting, both in the LoreBySpec voice — one file, one API key, two workflows | Live |
| [PostHog CSM Copilot](https://iamspecious.github.io/portfolio/posthog-cs-copilot.html) | Customer Success copilot for a 25–40 account portfolio: signal triage, response drafting, escalation strategy, and health assessment — encoded with PostHog's product suite, billing model, and customer type dynamics | In Progress |
| [PostHog CS Health Intelligence](./posthog-cs-health-intelligence/) | Research-backed prototype for the CS prioritisation gap: a structured account model and Python script that produces a ranked morning action list with plain-English reasons and suggested actions | Complete |
...
| [SaaS Integration Failure Diagnostic Guide](https://iamspecious.github.io/portfolio/support-guide.html) | Internal support enablement document covering webhook delivery failures, API authentication breakdowns, and downstream HRIS sync errors — written for Tier 1/2 agents, not developers | Complete |
...
Every AI tool project has a running dev log embedded directly in the portfolio — real build notes recording decisions, dead ends, pivots, and constraints as they happened. Not a polished write-up after the fact: the log is the artefact.
...
All AI tools are client-side only. No backend, no database, no server, no hosting cost.
...
The consistent architecture across all tools is intentional: same delivery mechanism, different problems solved.
